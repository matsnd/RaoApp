...
const columns: AnalyticsColumn[] = [
  ...
  { key: 'article_name', label: 'Usługa', sortable: true },
  ...
  return [
    {
      ...
      label: 'Usług zwykłych',
      sub: 'w umowach usługi (U)',
      icon: '🔧' as never,
      testId: 'kpi-svc-u-count',
    },
    {
      value: formatCurrency(totalRevenue.value),
      label: 'Przychód',
      sub: 'z usług zwykłych',
      variant: 'accent',
      icon: '💰' as never,
      testId: 'kpi-svc-u-revenue',
    },
    {
...
function onRowClick(row: AnalyticsRow): void {
  ...
  if (!serviceId) return
  openDrillDown('service', serviceId, String(row.article_name))
}
...
async function load(): Promise<void> {
  ...
  error.value = null
  try {
    const resp = await store.fetchPositions('all', props.dateFrom, props.dateTo, props.filters, undefined, 'desc', 'U')
...
<template>
  <div class="svc-tab" data-testid="services-regular-tab">
    <div v-if="loading && !data.length" class="svc-loading">
      Ładowanie usług zwykłych…
    </div>
    ...
    <template v-else-if="data.length">
      ...
      <div class="svc-section">
        <div class="svc-section-head">
          <span class="svc-section-title">🔧 Usługi zwykłe w umowach usługi ({{ filteredData.length }})</span>
          <div class="svc-actions">
            <input
              ...
              data-testid="svc-u-search"
            />
            <ExportCsvButton
              :columns="csvColumns"
              :rows="sortedRows"
              filename="uslugi_zwykle.csv"
            />
          </div>
        </div>
        <AnalyticsTable
    ...
    <div v-else class="svc-empty" data-testid="svc-u-empty">
      Brak danych o usługach zwykłych w wybranym okresie.
    </div>
  </div>
</template>
