...
class MockCondition:
    ...
    def __init__(self, period_from=None, period_to=None, rate1=None, rate2=None,
                 billing_label=None, period_count=None):
...
def test_cascading_3_conditions_matches_old_app():
    ...
    contract_type='S' (default) → stawka per doba (Z / unit)."""
    conditions = [
...
def test_cascading_single_condition():
    """Test with single closed condition (stawka per doba — Z / unit)."""
    conditions = [
...
def test_cascading_custom_billing_label():
    """Test with custom billing label (godzina, stawka per godzina).
    Legacy format: 'powyżej X godzin' for open-ended. Stawka → Z / unit."""
    conditions = [
...
def test_service_open_ended_from_zero():
    """Service condition (contract_type='U') with period_from=0 → 'do X godzin - {rate}zł' (no / unit)."""
    conditions = [
