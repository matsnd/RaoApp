...
2. ✅ **DONE (2026-07-05): `contractor_name: null` w /stats/currently-rented** — Root cause: router.py:241 brał `Contract.contractor_name` (snapshot, NULL dla umów z contractor_id) zamiast JOIN z contractors. FIX: `func.coalesce(Contractor.name, Contract.contractor_name)` + LEFT JOIN (outerjoin). Weryfikacja: API zwraca "Bud-Plus Sp. z o.o.", Playwright LiveFleet tab pokazuje kontrahenta.
...
4. ✅ **DONE (2026-07-05): /stats/currently-rented bez `is_settled==False` i bez `date_to IS NULL`** — rozliczone umowy liczone jako wynajęte; umowy na czas nieokreślony pomijane. FIX: dodano `(Contract.date_to.is_(None)) | (Contract.date_to >= today)` i `Contract.is_settled == False` (zgodnie z fleet-summary). Weryfikacja: API zwraca tylko 2 aktywne umowy (S001/2026).
...
**Faza 5 — Skeleton loaders + polish (MEDIUM impact, LOW effort)** — 0.5-1 dzień — ✅ DONE (2026-07-05)
- ✅ DashboardView (już SkeletonRow), AdminView, SettingsView, ArchiveView — skeleton rows zamiast "Ładowanie..." (TableSkeleton.vue)
- ✅ Komponent `<TableSkeleton :rows="5" :cols="4" layout="block|inline" />`
- ✅ View transitions (`<Transition name="fade" mode="out-in">` w App.vue, CSS z animations.css)
- ✅ Toast z ikoną (✓/✕/ℹ/⚠ — AppToast.vue, już zaimplementowane)
- ConfirmDialog z konkretnym obiektem ("Usuniesz umowę U/2024/123...") — osobny pod-zadanie
- ✅ Glossary skrótów (PNA, ZO, FA, OID, S/U) — GlossaryTip.vue + `<abbr title>` w ContractFormView/AnalyticsView/DashboardView/ArticleFormView
...
| RAO-P2-058 | Fakturownia — OID = numer umowy + mapowanie artykułów z metadanymi | P2 | L | team-verified | OID hybrydowe + product cache (11 produktów) + UI picker + sync-products + search endpoint + 28 testów PASS |
...
| RAO-P3-071 | Audyt UX — czytelność, spójność, przyjemność poruszania się | P3 | L | team-verified | Faza 1 (font-size 11px→13px, 10px→12px, 9px→11px, --font-size-xs 11→12; kontrast #718096→#5A6B7E WCAG AA; smallCount 171→0) + Faza 2 (7× toFixed→formatCurrency, 2× toLocaleDateString→formatDate w ContractFormView) + Faza 3 (a11y: global :focus-visible 2px primary + .btn/.page-btn jawnie; aria-label na icon buttons w AppToolbar/ContractFormView/ArticleFormView/ContractorFormView/ArchiveView/DashboardView/AppSidebar; for/id na labelach w LoginView/ContractFormView/ArticleFormView/ContractorFormView/ChangePasswordView/ResetPasswordView; role="alert" na error-message/field-error/form-error; aria-invalid na polach z błędem walidacji; role="dialog" aria-modal="true" + aria-labelledby na 13 modalach: ConfirmDialog, ContractFormView 8×, ContractorFormView, ArticleFormView 2×, LoginView, AdminView 2×, ConditionPanel, DrillDownDrawer, ArchiveView drill-drawer) + Faza 4 (CommissionView: hardcoded #1D2B53/#ccc/#27ae60 → var(--color-*) + .data-grid/.page-card/.btn; WorkerView: #0F234E→var(--color-primary), #F4F6FB→var(--color-bg-light), border-radius 10px→var(--border-radius); style.css martwy — focus-visible/skeleton-row/glossary-tip przeniesione do forms.css/animations.css) + Faza 5 (TableSkeleton.vue + zastąpiono "Ładowanie..." w SettingsView/ArchiveView/AdminView; view transitions fade w App.vue; GlossaryTip OID + abbr PNA/FA/ZO w ContractFormView/AnalyticsView) |
...
**Akcja:** Zostaw tylko SEC-001 (PDF ownership check). P2-046 usunięte z backlogu (2026-07-05).
