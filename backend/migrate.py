<<<<<<< C:/projects/repos/RaoApp/backend/migrate.py
"""
...
async def step5_service_fee_templates():
    """Build service_fee_templates — 6 standard fee types for both S and U contract types.
    Uses hardcoded standard values (verified against business requirements).
    Old firma.oplata_* columns had only 5 fees and no unit values — replaced with correct data.
    """
    print("[5/7] Building service fee templates …")
    conn = await aiomysql.connect(host=DB_HOST, port=DB_PORT, user=DB_USER, password=DB_PASS, db=DB_NAME)
    cur = await conn.cursor()

    # 6 standard fees — name, amount_from, amount_to, unit, description
    DEFAULT_FEES = [
    ...
    for contract_type in ("S", "U"):
        for i, (name, amt_from, amt_to, unit, desc) in enumerate(DEFAULT_FEES):
            await cur.execute("""
                INSERT INTO service_fee_templates
                    (company_id, contract_type, sort_order, name, amount_from, amount_to, unit, description, is_active)
                VALUES (1, %s, %s, %s, %s, %s, %s, %s, 1)
            """, (contract_type, i, name, amt_from, amt_to, unit, desc))
            count += 1
    await conn.commit()
    print(f"   {count} fee templates created (6 × S + 6 × U)")
...
if __name__ == "__main__":
    asyncio.run(main())
>>>>>>> C:/Users/mateu/.windsurf/worktrees/RaoApp/RaoApp-9c07f75b/backend/migrate.py
