...
class ContractService:
    ...
    async def apply_rate_preset_to_position(
        self, db: AsyncSession, pos_id: int, preset_id: int, replace: bool = True
    ) -> list[PositionCondition]:
        """Kopiuje warunki z cennika (ArticleRatePresetItem) do PositionCondition.

        Snapshot — brak FK do cennika. Edycja cennika NIE wpływa na istniejące umowy.
        Guard: contract.is_settled → 409.
        """
        from settings.models import ArticleRatePreset, ArticleRatePresetItem

        # Pobierz pozycję + umowę (guard is_settled)
        pos_result = await db.execute(
        ...
        contract = await self.get_contract(db, pos.contract_id)
        if contract.is_settled:
            raise conflict("Umowa jest rozliczona — modyfikacja warunków zablokowana.")

        # Pobierz cennik z items
        ...
        if replace:
            await db.execute(
                delete(PositionCondition).where(PositionCondition.position_id == pos_id)
            )
            await db.flush()

        created: list[PositionCondition] = []
        for item in sorted(preset.items, key=lambda i: i.sort_order):
    ...
    async def get_last_conditions_for_article(
        ...
        """Zwraca warunki z najnowszej umowy zawierającej pozycję z tym article_id.
        ...
        Zwraca None (router zwróci 404) jeśli brak historii.
        """
        from contracts.schemas import ConditionResponse
        from settings.models import RateType

        # Najnowsza umowa z pozycją dla tej maszyny
        stmt = (
            select(ContractPosition)
            .join(Contract, Contract.id == ContractPosition.contract_id)
            .where(ContractPosition.article_id == article_id)
            .order_by(Contract.created_at.desc(), ContractPosition.id.desc())
            .options(selectinload(ContractPosition.conditions))
            .limit(1)
        )
        result = await db.execute(stmt)
        ...
        contract_result = await db.execute(select(Contract).where(Contract.id == pos.contract_id))
        contract = contract_result.scalar_one_or_none()
        if not contract:
            return None

        # Batch-fetch rate types
        rt_ids = {c.rate_type_id for c in pos.conditions if c.rate_type_id}
        rt_map: dict[int, str] = {}
        if rt_ids:
            rt_result = await db.execute(
                select(RateType.id, RateType.name).where(RateType.id.in_(rt_ids))
            )
            rt_map = dict(rt_result.all())
        ...
        conditions = [
            ConditionResponse(
                ...
                rate_type_id=c.rate_type_id, rate_type_name=rt_map.get(c.rate_type_id),
                description=c.description, rate1=c.rate1, rate2=c.rate2,
                billing_label=c.billing_label, period_count=c.period_count,
                minimum=c.minimum,
            )
            for c in sorted(pos.conditions, key=lambda c: c.id)
        ]
        return {
            "source_contract_number": contract.number,
            "source_contract_date": contract.created_at,
