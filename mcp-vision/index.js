...
// === Vision providers ===
// Strategy: Nemotron Nano 12B v2 VL (free, OpenRouter) first → Claude (paid) fallback.
// Nemotron handles most UI checks; Claude kicks in when Nemotron's analysis is thin.
...
async function analyzeImage({ imageData, mediaType, prompt, metaLine }) {
  // 1) Free model (Nemotron) first
...
server.tool(
  "analyze_screenshot",
  "Analizuje screenshot UI aplikacji RAO. Najpierw darmowy Nemotron Nano 12B v2 VL (OpenRouter), fallback Claude Vision gdy free model nie poradzi. Zwraca ocenę: błędy wizualne, spójność design systemu, UX issues.",
  {
  ...
  async ({ image_path, question }) => {
    ...
    const result = await analyzeImage({ imageData, mediaType, prompt, metaLine: `**Plik:** ${image_path}` });
...
server.tool(
  ...
  "Robi screenshot podanego URL przez Playwright, następnie analizuje go. Najpierw darmowy Nemotron Nano 12B v2 VL (OpenRouter), fallback Claude Vision. Wymaga działającego frontendu.",
  {
